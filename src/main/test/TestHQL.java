...
public class TestHQL {
    ...
    public void list09() {
        ...
        try {
            session = HibernateUtil.openSession();
            List<Student> stus = session.createQuery("select stu from Student stu where stu.classRoom.id=?")
                    .setParameter(0, 1)
                    .list();
            for (Student stu : stus) {
    ...
    public void limit11() {
        ...
        try {
            ...
            List<Student> students = session.createQuery
                    ("select stu from Student stu where stu.classRoom.id in(:cla)")
    ...
    public void dto15() {
        ...
        try {
             ...
             * setParameterList可以替代一组数据
             */
            List<StudentDto> studentDtos = session.createQuery("select new tedu.sharkj.dto.StudentDto(stu.name,cla.name as cname,spe.name as pname)" +
                    "from Student stu left join stu.classRoom cla left join cla.special spe where cla.id=:id")
